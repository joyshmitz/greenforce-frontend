...
function getCalculation() {
  return mockGetCalculationByIdQuery(async ({ variables }) => {
    ...
    return calculationById
      ? HttpResponse.json({ data: { __typename: 'Query', calculationById } })
...
function getCalculations() {
  return mockGetCalculationsQuery(async ({ variables }) => {
    if (!variables.executionTime) {
      return HttpResponse.json({ data: undefined }, { status: 500 });
    }

    await delay(delayInMs);

    return HttpResponse.json({ data: { __typename: 'Query', calculations: mockedCalculations } });
  });
}
...
function getGridAreas() {
  return mockGetGridAreasQuery(async () => {
    ...
    return HttpResponse.json({ data: { __typename: 'Query', gridAreas: mockedGridAreas } });
  });
}
